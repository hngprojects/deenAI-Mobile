...
const SURAH_METADATA: Surah[] = [
  ...
  },
  {
    ...
    englishNameTranslation: "Those Ranges in Ranks",
    numberOfAyahs: 182,
    revelationType: "Meccan",
  },
  {
        ...
        quranArabicRaw.data.surahs.forEach((surah: any) => {
          ...
          if (surah.ayahs && Array.isArray(surah.ayahs)) {
            verses[surahNumber] = surah.ayahs.map((ayah: any) => ({
              ...
              transliteration: this.getTransliteration(
                surahNumber,
                ayah.numberInSurah
              ),
            }));
  ...
  private getTranslation(surahNumber: number, verseNumber: number): string {
    try {
      if (quranTranslationRaw.data?.surahs) {
        const surah = quranTranslationRaw.data.surahs.find(
          (s: any) => s.number === surahNumber
        );
        if (surah?.ayahs) {
          const ayah = surah.ayahs.find(
            (a: any) => a.numberInSurah === verseNumber
          );
          return ayah?.text || "";
        }
      }
  ...
  private getTransliteration(surahNumber: number, verseNumber: number): string {
    try {
      if (quranTransliterationRaw.data?.surahs) {
        const surah = quranTransliterationRaw.data.surahs.find(
          (s: any) => s.number === surahNumber
        );
        if (surah?.ayahs) {
          const ayah = surah.ayahs.find(
      ...
      console.warn(
        `Transliteration not found for ${surahNumber}:${verseNumber}`
      );
      return "";
    }
  }
